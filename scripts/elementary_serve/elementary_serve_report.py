...
def download_file():
    try:
        ...
        if cached_data:
            logger.info("Serving report from cache")
            return send_file(
                BytesIO(cached_data), as_attachment=True, download_name=blob_name
            )

        logger.info("Downloading and serving report from blob storage")
        ...
        cache.set(
            blob_name, file_content, timeout=300
        )  # Cache timeout is set to 300 seconds (5 minutes)

        # Send the file in the response
        return send_file(
            BytesIO(file_content), as_attachment=False, download_name=blob_name
        )
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("ELEMENTARY_SERVE_PORT", "8080"))
    host="0.0.0.0"

    logger.info(f"Serving elementary report on: http://{host}:{port}")
